...
final class WPUserShortcode {
        ...
        if (isset($atts['id']) && !empty($atts['id'])) {
            ...
            echo '<div class="box box-default">
                    <div class="box-header" role="tab" id="headingOne">
                        <h3 class="box-title">
                            <a role="button" data-toggle="collapse" data-parent="#accordion" href="#my_accout_'.esc_attr($atts['id']).'" aria-expanded="true" aria-controls="collapseOne">
                           '.get_the_title($atts['id']).'
                            </a>
                        </h3>
                    </div>
                    <div id="my_accout_'.esc_attr($atts['id']).'" class="box-collapse collapse in" role="tabpanel" aria-labelledby="headingOne" aria-expanded="true">';
                        if(!(isset($_GET['user_id']) || (isset($atts['type']) && 'register' == $atts['type']))) {
                            echo '<div style="display: none;" id="wpuser_errordiv_form_'.esc_attr( $atts['id']).'"class="alert alert-dismissible" role="alert">
                                     <label id="wpuser_error_register_'.esc_attr( $atts['id'] ).'"></label>
                                    </div>
                            <form method="post" id="wpuser_form_' . esc_attr( $atts['id'] ). '" onsubmit="return false">
                            <input name="wpuser_update_setting" type="hidden" value="' . wp_create_nonce('wpuser-update-setting') . '"/>
                            <input name="wpuser_form_id" type="hidden" value="' . esc_attr( $atts['id'] ). '"/>
                            ...
                            if ($userplus_field_order) {
                                ...
                                for ($i = 0; $i < $fields_count; $i++) {
                                    ...
                                    if(!isset($_GET['user_id'])) {
                                        ...
                                        $viewFields = profileController::view_fields($key, $array, $wp_user_appearance_skin, $atts['id'], sanitize_text_field($_GET['user_id']));
    ...
    function wp_user_search($atts) {
        ...
        echo '<style>';
            echo '#advanced_filter.form-horizontal .form-group {
    ...
    $login_redirect = "";
    $current_form_id = isset($_GET['form_id']) && !empty($_GET['form_id']) ? sanitize_text_field( $_GET['form_id'] ): 0;
    $login_redirect = apply_filters('login_redirect', $login_redirect, $current_form_id);
    if(!empty($login_redirect)){
      $atts['login_redirect'] = $login_redirect;
    }
    ...
    echo "<script>
        function delete_img(img_delete, image = 'image') {
           ...
           jQuery('#img_view'+img_delete).attr('src','".esc_url(WPUSER_PLUGIN_URL) ."assets/images/'+image+'.png');
        ...
        if (is_user_logged_in()) {
          ...
          if ( false == empty($wp_user_page) ){
            $genre_url = get_permalink($wp_user_page) ;
              echo '<a href="' .esc_url ( $genre_url ). '" title=""><span class="fa fa-user"></span> ';
...
/*jQuery(document).ready(function(){
 ...
 <div class="wpuser_view_filter panel panel-default">
                                ...
                                foreach ($chunk_list as $value) {
                                    ...
                                    if (isset($atts['size']) && $atts['size'] == 'small') {
                                                    ...
                                                    <div class="box box-primary wpuser-custom-box">
                                                        <div class="box-body box-profile" style="padding:0px !important">
                                                            ...
                                                            <div style="margin: 10px;"  class="media-left pos-rel col-md-3">
                                                                <a> <img class="img-circle img-xs" src="' . esc_url ( $wp_user_profile_img ). '" width="40px" alt="Profile Picture"></a>
                                                                <i class="badge badge-success badge-stat badge-icon pull-left"></i>
                                                            </div>
                                                            <div class="media-body">
                                                                <div class="pull-left"><h5 class="member_list_display_name mar-no"><a href="'.esc_url( $genre_url ).'" >' . esc_attr( $name ). '</a></h5>
                                        ...
                                        echo '<div class="box box-primary wpuser-custom-box">
                                                <div class="box-body box-profile">
                                                    <div class="media-left pos-rel col-md-3">
                                                        <a> <img class="img-circle img-xs" src="' . esc_url( $wp_user_profile_img ). '" width="70px" alt="Profile Picture"></a>
                                                        <i class="badge badge-success badge-stat badge-icon pull-left"></i>
                                                    </div>
                                                    <div class="media-body">
                                                        <h3 class="member_list_display_name mar-no"><a href="'.esc_url( $genre_url ).'" >' . esc_attr( $name ). '</a></h3>
                                                        <small class="text-muted">' . esc_attr( $title ) . '</small>
                                                        <br>
                                                        <h3>';
                                        if ($user_status == 0) {
                                        ...
                                        if (!empty($user_mobile)) {
                                            echo '<a href="tel:' . esc_attr( $user_mobile ). '" data-toggle="tooltip"  data-original-title="' . esc_attr( $user_mobile ). '" title="' . esc_attr( $user_mobile ). '"><i class="fa fa-phone"></i></a>&nbsp;&nbsp;';
                                        }
                                        ...
                                        if (!empty($user_blog_url)) {
                                            echo '<a href="' . esc_url( $user_blog_url ). '" target="_blank" data-toggle="tooltip"  data-original-title="Blogs" title="Blogs"><i class="fa fa-th-large"></i></a>&nbsp;&nbsp;';
                                        }
                                        do_action('wp_user_hook_member_list_icon', $info);
                                        echo '<hr>
                                    </h3>
                                    <a type="button" class="btn ' . esc_attr( $wp_user_appearance_button_type ). ' btn-default col-md-5" href="'.esc_url( $genre_url ).'" >View Profile</a>
