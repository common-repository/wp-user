...
class wpuserwidget extends WP_Widget {
        ...
        if (is_user_logged_in()) {
            ...
            if ($wp_user_avatar_enable) {
                echo '<div class="col-md-3">
                             <img class="wpuser_profile_img img-responsive img-circle" style="width:50px" src="' . esc_url( $wp_user_profile_img ). '" alt="User Avatar">                                             
                             </div>';
            }
            echo '<div class="col-md-9"> 
                            <h3 class="profile-username"> <a href="' . get_permalink(get_option('wp_user_page')) . '" title="My Account">' . esc_attr( $current_user->user_login ) . '</a></h3>';
    ...
    public function form($instance)
    {
        ...
        ?>
        <p>
            <label for="<?php echo esc_attr($this->get_field_id('title')); ?>"><?php _e('Logged-out title'); ?></label>
        ...
        <p>
            <label
                for="<?php echo esc_attr($this->get_field_id('title_logged_in_user')); ?>"><?php _e('Logged-in title'); ?></label>
            <input class="widefat" id="<?php echo esc_attr($this->get_field_id('title_logged_in_user')); ?>"
                   name="<?php echo esc_attr($this->get_field_name('title_logged_in_user')); ?>" type="text"
                   value="<?php echo esc_attr($title_logged_in_user); ?>"/>
        </p>

        <p>
            <input class="checkbox" type="checkbox" <?php checked($wp_user_register_enable, 'on'); ?>
                   id="<?php echo esc_attr($this->get_field_id('wp_user_register_enable')); ?>"
                   name="<?php echo esc_attr($this->get_field_name('wp_user_register_enable')); ?>"/>
            <label
                for="<?php echo esc_attr($this->get_field_id('wp_user_register_enable')); ?>"><?php _e('Show Register tab'); ?></label>
        </p>

        <p>
            <input class="checkbox" type="checkbox" <?php checked($wp_user_forgot_enable, 'on'); ?>
                   id="<?php echo esc_attr($this->get_field_id('wp_user_forgot_enable')); ?>"
                   name="<?php echo esc_attr($this->get_field_name('wp_user_forgot_enable')); ?>"/>
            <label
                for="<?php echo esc_attr($this->get_field_id('wp_user_forgot_enable')); ?>"><?php _e('Show forgot password tab'); ?></label>
        </p>

        <p>
            <input class="checkbox" type="checkbox" <?php checked($wp_user_icon_enable, 'on'); ?>
                   id="<?php echo esc_attr($this->get_field_id('wp_user_icon_enable')); ?>"
                   name="<?php echo esc_attr($this->get_field_name('wp_user_icon_enable')); ?>"/>
            <label
                for="<?php echo esc_attr($this->get_field_id('wp_user_icon_enable')); ?>"><?php _e('Show field icons'); ?></label>
        </p>

        <p>
            <input class="checkbox" type="checkbox" <?php checked($wp_user_avatar_enable, 'on'); ?>
                   id="<?php echo esc_attr($this->get_field_id('wp_user_avatar_enable')); ?>"
                   name="<?php echo esc_attr($this->get_field_name('wp_user_avatar_enable')); ?>"/>
            <label
                for="<?php echo esc_attr($this->get_field_id('wp_user_avatar_enable')); ?>"><?php _e('Show logged-in user avatar'); ?></label>
        </p>
        <?php
    }
