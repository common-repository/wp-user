...
<div class="row" id="profile_view">
  <?php if(isset($_GET['redirect']) && !empty($_GET['redirect'])){
    $url_title=(isset($_GET['url_title']) && !empty($_GET['url_title'])) ? sanitize_text_field( $_GET['url_title'] ): 'Back';
    ?>
      <div class="col-md-12">
        <a type="button" id="member_list_button" href="<?php echo esc_url( $_GET['redirect'] )?>" title="<?php _e($url_title,'wpuser')?>" class="pull-right btn btn-default btn-flat">
  ...
  <div class="col-md-12 wpuser-center margin-top-15">
        ...
        <div class="wpuser-custom-box" style="border-top:none">
          <img id="wpuser_profile_image" class="profile-user-img img-circle" src="<?php echo esc_url($data['wp_user_profile_img'])?>" alt="User profile picture">
...
<br>
        ...
        <div  class="wpuser-post col-sm-9 col-md-5 col-xl-6 mb-6 text-left">
          ...
          <?php if(!empty($data['user_info'])) { ?>
           <p class="profile-label text-left" id="wpuser_profile_label"> <a href="<?php echo esc_url( $data['user_info']['Website'])?>" target="_blank"><?php _e($data['user_info']['Website'],'wpuser')?></a></p>
        <?php } ?>
        </div>
        ...
        <ul class="nav">
          <?php foreach ($wp_view_user_profile as $tab => $user_profile) {
            if( false == empty($user_profile['tab'])){
              $active=(isset($_GET['tab_active'])) ? '' : sanitize_text_field( $user_profile['active'] );
              ...
              echo ' <li class="' .esc_attr( $active ). '" id="tab_'.esc_attr( $tab).'">><a id="tab_link_'.esc_attr($tab).'" class="'. esc_attr( $tab ).'" href="'.esc_url( $href ).'" '.wp_kses_post( $data_toggale).'>' . esc_attr( $user_profile['tab'] ). '</a></li>';
            }
          } ?>
        </ul>
        <!-- /.box-body -->
