...
$html .='<div class="modal fade" style="overflow: scroll;margin: auto" id="wpuser_myModal" tabindex="-1" role="dialog" aria-labelledby="myModalLabel">
        ...
        $html .='</form>
                    ...
                    <div class="modal-footer">
                        <button type="button" class="btn '.sanitize_text_field( $wp_user_appearance_button_type ).' btn-default"
                                data-dismiss="modal">
                                    '.__('Close', 'wpuser').'
</button>
                        <button type="button" id="wpuser_send_mail"
                                class="wpuser_button btn '.sanitize_text_field( $wp_user_appearance_button_type ).' btn-primary wpuser-custom-button">
'.__('Send', 'wpuser') .'
</button>
                    </div>
                </div>
        ...
        $html .='<form id="wpuser_filter_member_list_form" class="wpuser_filter_member_list_form form-horizontal">
                          <div class="box-body">
                            <div class="col-sm-12 col-md-12">
                                <div class="form-group">
                                    <div class="col-sm-12">
                                        <input type="text" class="form-control" name="search_user" value="'. sanitize_text_field( $strSearch ).'" id="search_user" placeholder="search">
                                    </div>
                                </div>
                            </div>
                            <div class="col-sm-12">
                            ...
                            if((isset($atts['key']) && isset($atts['key']) && !empty($atts['value']) && !empty($atts['value'])) ||
                                ...
                                $arrKey = (isset($_GET['key']) && !empty($_GET['key'])) ? sanitize_text_field( $_GET['key'] ): $atts['key'];
                                $arrValue = (isset($_GET['value']) && !empty($_GET['value'])) ? sanitize_text_field( $_GET['value'] ): $atts['value'];
                                $html .='<input name="key" type="hidden" value="' . sanitize_text_field( $arrKey ). '"/>';
                                ...
                                if ( count( $ids_array ) == count( $text_array ) ){
                                    ...
                                    foreach ( $combined_array as $k => $v ){
                                        $html .='<input name="' . sanitize_text_field( $k ) . '" type="hidden" value="' . sanitize_text_field( $v ). '"/>';
                                    }
                                }
                            }

                            if (isset($atts['role_in']) && !empty($atts['role_in'])) {
                                $html .='<input type="hidden" value="'.sanitize_text_field( $atts["role_in"] ).'" name="role_in">';
                            }
                            if (isset($atts['role_not_in']) && !empty($atts['role_not_in'])) {
                                $html .='<input type="hidden" value="'.sanitize_text_field( $atts["role_not_in"] ).'" name="role_not_in">';
                            }
                            if (isset($atts['include']) && !empty($atts['include'])) {
                                $html .='<input type="hidden" value="'.sanitize_text_field( $atts["include"] ).'" name="include">';
                            }
                            if (isset($atts['exclude']) && !empty($atts['exclude'])) {
                                $html .='<input type="hidden" value="'.sanitize_text_field( $atts["exclude"] ).'" name="exclude">';
                            }
                            if (isset($atts['order']) && !empty($atts['order'])) {
                                $html .='<input type="hidden" value="'.sanitize_text_field( $atts["order"] ).'" name="order">';
                            }
                            if (isset($atts['include']) && !empty($atts['include'])) {
                                $html .='<input type="hidden" value="'.sanitize_text_field( $atts["include"] ).'" name="include">';
                            }
                            if (isset($atts['approve']) && !empty($atts['approve'])) {
                                $html .='<input type="hidden" value="'.sanitize_text_field( $atts["approve"]).'" name="approve">';
                            }

                            if (isset($atts['profile_form_id']) && !empty($atts['profile_form_id'])) {
                                $html .='<input type="hidden" value="'.sanitize_text_field( $atts["profile_form_id"]).'" name="profile_form_id">';
                            }
                            ...
                            if(isset($customFilter) && !empty($customFilter)){
                                    foreach ( $customFilter as $key ){
                                        if(!empty($key) && isset($_GET[$key]) && !empty(($_GET[$key])) && 'null' != $_GET[$key]){
                                          $html .='<input type="hidden" value="'.sanitize_text_field( $_GET[$key] ).'" name="'.sanitize_text_field( $key ).'">';
                                        }
                                    }
                            }
        ...
        if (isset($atts['id']) && !empty($atts['id'])) {
            $html .= '<input type="hidden" value="'.sanitize_text_field( $atts['id'] ).'" name="form_id">
            <div class="row">
                                                            <a class="pull-right advance_filter" data-toggle="collapse" href="#collapseFilter" aria-expanded="true" aria-controls="collapseExample">
            ...
            if (isset($_GET['form_id']) && !empty($_GET['form_id'])) {
                $html .= '<input type="hidden" value="'.sanitize_text_field( $_GET['form_id']).'" name="search_form_id">';
                $userplus_field_order = get_post_meta(sanitize_text_field( $_GET['form_id']), 'userplus_field_order', true);
                $form_fields = get_post_meta(sanitize_text_field( $_GET['form_id']), 'fields', true);;
                if ($userplus_field_order) {
                    $fields_count = count($userplus_field_order);
                    for ($i = 0; $i < $fields_count; $i++) {
                        $key = $userplus_field_order[$i];
        ...
        $html .= '</div>
                        ...
                        <div class="">
                                ...
                                <button type="button" id="wpuser_filter_member_list_clear" class="wpuser_filter_member_list_clear btn btn-flat btn-default">Clear</button>
