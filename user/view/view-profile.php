  ...
  ?>
    ...
    <div class="col-md-12 margin-top-15 wpuser-profile-header">
        <div class="col-md-2 wpuser-center">
            <img id="wpuser_profile_image" class="wpuser_viewimage profile-user-img" src="<?php echo esc_url( $data['wp_user_profile_img'] )?>"
                 alt="Profile Picture">
              ...
              <ul class="row nav user_menu text-left">
                  <?php foreach ($wp_view_user_profile as $tab => $user_profile) {
                    if( false == empty($user_profile['tab'])){
                        ...
                        echo ' <li class="menu_item col-md-12 col-sm-4 ' .esc_attr( $active ). '" id="tab_'.esc_attr( $tab ).'"> <a id="tab_link_'.esc_attr( $tab ).'" class="'. esc_attr( $tab ).'" href="'.esc_url( $href ).'" '.wp_kses_post( $data_toggale ).'><spam class="user_menu_icon '.esc_attr( $icon ).'"></spam> <spam class="menu_list">' . esc_attr( $user_profile['tab'] ). '</spam></a></li>';
                    }
                  } ?>
              </ul>
              <!-- /.box-body -->
