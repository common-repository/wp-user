<?php
        ...
        $html .='<form id="wpuser_filter_member_list_form" action="' . get_permalink(get_option('wp_user_page')) . '" method="get" class="wpuser_filter_member_list_form">
                          <div class="box-body">';
        ...
        $html .='<div class="serch-user col-sm-12 col-md-12">
                            ...
                            if (isset($atts['orderby']) && !empty($atts['orderby'])) {
                                $html .='<input type="hidden" value="'.sanitize_text_field( $atts["orderby"] ).'" name="orderby">';
                            }
                            ...
                            if (isset($atts['id']) && !empty($atts['id']) && isset($atts['is_filter']) && !empty($atts['is_filter'])) {
                                $html .='<input type="hidden" value="'.sanitize_text_field( $atts["id"] ).'" name="wp_user_search">';
                            }
        ...
        if (isset($atts['id']) && !empty($atts['id'])) {
          ...
          //  $collapse_in = ( 'header' != $search_type ) ? '' : 'in';
            $collapse_in = 'in';
            $html .= '<div class="collapse '.sanitize_text_field( $collapse_in ).'" id="collapseFilter'.sanitize_text_field( $form_id ).'" aria-expanded="true">
            ...
            if( 'header' == $search_type  ) {
            $html .= '
                    <div class="col-xs-12 col-md-'.sanitize_text_field( $divCol ).' col-sm-12 ">
                                <input type="submit" class="btn btn-flat btn-primary btn-'.sanitize_text_field( $inputType ).' btn-block" value="'.__('Search','wpuser').'" aria-label="Close"> ';
                $html .= '  </div>';
            }
            $html .= '
                    </div>
        ...
        if(isset($atts['key']) && isset($atts['key']) && !empty($atts['value']) && !empty($atts['value'])){
            $html .='<input name="key" type="hidden" value="' . sanitize_text_field( $atts['key'] ). '"/>';
        ...
        if( 'header' != $search_type  ) {
            $html .= '
                            <div class="col-sm-12 col-md-'.sanitize_text_field( $divCol ).'">
                                        <input type="submit" class="btn btn-flat btn-primary" value="'.__('Search','wpuser').'" aria-label="Close"> ';

            $html .= ' <button type="button" id="wpuser_filter_member_list_clear" class="wpuser_filter_member_list_clear btn btn-flat btn-default">Clear</button>';
            $html .= '  </div>';
        }
