...
if (!class_exists('wpuserAjax')) :
    class wpuserAjax
    {
        ...
        public static function wpuser_register( $params= array() )
        {
            ...
            $wp_user_email_name = (isset($data['user_login'])) ? sanitize_text_field( $data['user_login'] ): ((isset($_POST['user_login'])) ? sanitize_text_field( $_POST['user_login'] ) : '');
            $wp_user_email = (isset($data['user_email'])) ? sanitize_email( $data['user_email'] ): ((isset($_POST['user_email'])) ? sanitize_email( $_POST['user_email'] ): '');
            ...
            if (isset($_POST['wpuser_form_id']) && !empty($_POST['wpuser_form_id'])) {
              $arrUserMeta['wpuser_form_id'] = sanitize_text_field( $_POST['wpuser_form_id'] );

              $customFilter =  apply_filters('wp_user_register_form_filter', []);

              if(isset($customFilter) && !empty($customFilter)){
                  if( !empty($customFilter) ){
                      foreach ( $customFilter as $key ){
                          if(!empty($key) && isset($_POST[$key]) && !empty(($_POST[$key])) && 'null' != $_POST[$key]){
                              $arrUserMeta[$key] = $_POST[$key];
                          }
                      }
                  }
                ...
                $form_role = get_post_meta(sanitize_text_field( $_POST['wpuser_form_id']), 'userplus_form_role', true);
                $form_role = (isset($_POST['role']) && !empty($_POST['role'])) ? sanitize_text_field( $_POST['role'] ): $form_role;
                ...
                if (!isset($_POST['user_email'])) {
                    ...
                    if (!empty($_POST['user_email'])) {
                        ...
                        if (!filter_var($email, FILTER_VALIDATE_EMAIL)) {
                            ...
                            $result['status'] = 'warning';
                            $validationArr['user_email'] = __('Invalid email format', 'wpuser');
                            $result['error'] = $validationArr;
                            print_r(json_encode($result));
                            exit;
                        }
                    }
                ...
                if (username_exists($_POST['user_login'])) {
                    ...
                    $validationArr['user_login'] = __('The username already exists', 'wpuser');
                    $result['error'] = $validationArr;
                    print_r(json_encode($result));
                    exit;
                }
                if (username_exists($_POST['user_email'])) {
                    ...
                    $validationArr['user_email'] = __('The email address already exists', 'wpuser');
                    $result['error'] = $validationArr;
                    print_r(json_encode($result));
                    exit;
                }

                $userplus_field_order = get_post_meta(sanitize_text_field($_POST['wpuser_form_id']), 'userplus_field_order', true);
                ...
                if ($userplus_field_order) {
                    ...
                    for ($i = 0; $i < $fields_count; $i++) {
                        ...
                        if( true == isset($_POST[$array['meta_key']])) {
                            $arrUserMeta[$array['meta_key']] = sanitize_text_field( $_POST[$array['meta_key']] );
                        }
                ...
                $validationArr = apply_filters('wpuser_validation_filter', $_POST);
                ...
                if (isset($wp_user_tern_and_condition) && $wp_user_tern_and_condition == 1) {
                    $wp_user_term_condition = (isset($data['wp_user_term_condition'])) ? $data['wp_user_term_condition'] : ((isset($_POST['wp_user_term_condition'])) ? wp_kses_post( $_POST['wp_user_term_condition'] ): '');
                ...
                if (username_exists(sanitize_email( $_POST['user_email']))) {
                    $result['status'] = 'warning';
                    $result['message'] = __('The email address already exists', 'wpuser');
                    print_r(json_encode($result));
                    exit;
        ...
        public static function send_otp_user_email( $user_email, $intOTP, $user, $intTimeValidate = 15 ){
          ...
          if (get_option('wp_user_disable_login_otp_link') != 1) {
            $intOTPText = '<a href="'.admin_url('admin-ajax.php') . '?action=wpuser_link_login&email=' . $user_email.'&otp='.$intOTP.'">'.$intOTP."</a>";
          } else {
            $intOTPText = $intOTP;
          }
        ...
        public static function getUserList( $params = array() )
        {
            ...
            $role__in = (isset($_POST['role_in']) && !empty($_POST['role_in'])) ? explode(',', sanitize_text_field( $_POST['role_in'] )) : array();
            $role__not_in = (isset($_POST['role_not_in']) && !empty($_POST['role_not_in'])) ? explode(',', sanitize_text_field( $_POST['role_not_in'] )) : array();
            $include = (isset($_POST['include']) && !empty($_POST['include'])) ? explode(',', sanitize_text_field( $_POST['include']) ) : array();
            ...
            $per_page = ((isset($_POST['per_page']) && !empty($_POST['per_page']))) ? sanitize_text_field( $_GET['per_page'] ): 12;
            $offset = (($page - 1) * $per_page);

            if(isset($_POST['form_id']) && !empty($_POST['form_id'])){
              $form_role = get_post_meta( sanitize_text_field( $_POST['form_id'] ), 'userplus_form_role', true);
              $role__in = (!empty($form_role)) ? $form_role : $role__in;
            }
            ...
            $meta_query = array();
            if(isset($_POST['form_id']) && !empty($_POST['form_id'])){
                $userplus_field_order = get_post_meta(sanitize_text_field( $_POST['form_id']), 'userplus_field_order', true);
                if ($userplus_field_order) {
                    $fields_count = count($userplus_field_order);
                    for ($i = 0; $i < $fields_count; $i++) {
                        $key = $userplus_field_order[$i];
            ...
            if(isset($_POST['search_form_id']) && !empty($_POST['search_form_id'])){
                $userplus_field_order = get_post_meta(sanitize_text_field( $_POST['search_form_id']), 'userplus_field_order', true);
                if ($userplus_field_order) {
                    $fields_count = count($userplus_field_order);
                    for ($i = 0; $i < $fields_count; $i++) {
                        $key = $userplus_field_order[$i];
                        if(!empty($key) && isset($_POST[$key]) && !empty(($_POST[$key]) && 'null' != $_POST[$key]) && $key =='age' ){
                          $age = sanitize_text_field($_POST[$key]);
                            $meta_query[] = array(
                                'key' => 'dob',
                                'value' => array(date('Y-m-d'), date('Y-m-d', strtotime("$age year"))),
                                 'compare' => 'BETWEEN',
                                 'type' => 'DATE'
                            );
                        } else if(!empty($key) && isset($_POST[$key]) && !empty(($_POST[$key]) && 'null' != $_POST[$key]) ){
                            $meta_query[] = array(
                                'key' => $key,
                                'value' => sanitize_text_field($_POST[$key]),
                                'compare' => '=='
            ...
            if(isset($_POST['key']) && !empty($_POST['key'])){
                $arrStrKeyFilter = (isset($_POST['key']) && !empty($_POST['key']) ) ? explode(',', sanitize_text_field( $_POST['key'] )) : array();
            ...
            $customFilter =  apply_filters('wpuser_advananced_search_filter', []);
            ...
            if(empty($meta_query) && isset($_POST['search_user']) &&  !empty($_POST['search_user']) && 'null' != $_POST['search_user'] ){
                if( is_numeric( $_POST['search_user'] ) ){
                    $args['include'] = array(  sanitize_text_field($_POST['search_user']) );
                }else {
            ...
            foreach ($blogusers as $value) {
                ...
                $header_block_info = array();
                $header_block_info = apply_filters('wpuser_filter_list_icon', $header_block_info, $value->ID);
                ...
                $result['user_body_custom_html']= $user_body_custom_html;
        ...
        public static function wpuser_contact( $params = array() )
        {
            ...
            if (!empty($WP_USER_INPUT['wp_user_email_subject']) && !empty($WP_USER_INPUT['wp_user_email_content'])) {
                ...
                $message = sanitize_text_field($WP_USER_INPUT['wp_user_email_content']);

                $message .= '<br>Username: ' . $current_user->user_login . '<br />';
                $message .= 'User email: ' . $current_user->user_email . '<br />';

                if (!(wp_mail($email, sanitize_text_field($WP_USER_INPUT['wp_user_email_subject']), $message, $headers)))
        ...
        public static function wpuser_send_mail_action( $params = array())
        {
            ...
            $user_info = get_userdata(sanitize_text_field( $_POST['id'] ));

            $to = $user_info->user_email;
            $subject = (isset($_POST['subject']) && !empty($_POST['subject'])) ? sanitize_text_field( $_POST['subject'] ): "New Message From " . get_bloginfo('name');
            ...
            if (isset($_POST['from']) && !empty($_POST['from'])) {
                $headers[] = 'From: ' . get_bloginfo('name') . ' < ' . sanitize_text_field( $_POST['from'] ). '>' . "\r\n";
            }
        ...
        public static function wpuser_activation( $params = array() )
        {
          if( !empty($params)){
            ...
            $wp_user_email_name = ((isset($_REQUEST['email'])) ? sanitize_email($_REQUEST['email']) : '');
        ...
        public static function wpuser_link_login( $params = array() )
        {
          if( !empty($params)){
            ...
            $wp_user_email_name = ((isset($_REQUEST['email'])) ? sanitize_email($_REQUEST['email']) : '');
            $key = ((isset($_REQUEST['key'])) ? sanitize_text_field($_REQUEST['key']) : '');
            $wp_user_otp = ((isset($_REQUEST['otp'])) ? sanitize_text_field($_REQUEST['otp']) : '');
            $wp_user_redirect = ((isset($_REQUEST['redirect'])) ? esc_url_raw($_REQUEST['redirect']) : get_permalink(get_option('wp_user_page')));
            if (!empty($wp_user_email_name)) {
                if (filter_var($wp_user_email_name, FILTER_VALIDATE_EMAIL)) {
        ...
        public static function wpuser_delete_notification(){
            ...
            if($_POST['id']==0){
               $delete=array( 'recipient_id' => get_current_user_id() );
                ...
                $delete= array( 'ID' => sanitize_text_field( $_POST['id'] ));
            }
