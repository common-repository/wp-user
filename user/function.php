...
class profileController {
    ...
    public static function view_fields($key, $array, $form_type = 'default', $form_id = 0, $user_id = null)
    {
        ...
        if ($form_type == 'float') {
           ...
           // $fieldClass= 'col-xs-8 col-sm-12';
            $field .= '<span class="col-md-'.$rowClass.' col-sm-12">';
        }else if ($form_type == 'block-2') {
           $rowClass = ($array['type'] == 'checkbox') ? 12 : $rowClass ;
           $field .= '<span class="col-md-'.$rowClass.' col-sm-12">';
       }else{
                ...
                if (isset($array['edit_choices'])) {
                    ...
                    foreach ($array['edit_choices'] as $v) {
                        ...
                        if ((is_array($value) && in_array($v, $value)) || $v == $value) {
                            $field .= "<span class='col-xs-12 col-md-6 col-sm-12 " .$fieldClass." ". $array['meta_key'] . "'>  ".$v."</span>";
                        }
                    }
                }
                break;
    ...
    public static function edit_fields($key, $array, $form_type = 'default', $form_id = 0, $user_id = null, $form = 'default', $type = 'default')
    {
        ...
        if (isset($user_id) & $user_id!=null) {
            ...
            if ($array['type'] == 'image_upload' && !in_array($array['meta_key'],array('user_meta_image','profile_pic'))) {
                ...
                $image ='<div class="col-sm-12">
                      <img class="img-responsive user-img profile_background_pic" id="view_'.sanitize_text_field( $array['meta_key'] ).'" src="'. esc_url_raw( $value ). '"  alt="Image">
                    </div>';
            }
            if (isset($array['user_edit']) && $array['user_edit'] =='0' && is_user_logged_in()) {
                $readonly = 'readonly';
            }
        ...
        $divCol = ($array['type'] == 'checkbox') ? 12 : $divCol ;
        $form_type = ($form_type == 'block-2' ) ? 'block' : $form_type ;
        ...
        if ($form_type == 'float' || $form_type == 'block' || $form_type == 'rounded') {
            ...
            if ($form_type == 'rounded' && !$wp_user_appearance_icon && !empty($array['icon'])) {
                ...
                $field .= '<span class="fa ' . sanitize_text_field( $array['icon'] ). '"></span> ';
                $field .= '</span>';
            }
            $field .= isset($array['label']) ? $array['label'] :'';
        ...
        switch ($array['type']) {
            ...
            case 'text':
                ...
                if (!$wp_user_appearance_icon && !empty($array['icon']) && $form_type != 'rounded') {
                    $field .= '<span class="fa ' . sanitize_text_field( $array['icon'] ). ' form-control-feedback"></span>';
                }
                break;
            ...
            case 'date':
                $field .= "<input $style type='date' $is_required class='wpuser_date form-control' name='" . $array['meta_key'] . "' id='" . $elementId . "' value='" . $value . "' placeholder='" . $array['placeholder'] . "' $data $readonly />";
                if (!$wp_user_appearance_icon && !empty($array['icon']) && $form_type != 'rounded') {
                    $field .= '<span class="fa ' . $array['icon'] . ' form-control-feedback"></span>';
                }
                break;
            ...
            case 'checkbox':
                ...
                if (isset($array['edit_choices'])) {
                    ...
                    foreach ($array['edit_choices'] as $v) {
                        $v = stripslashes($v);
                        $field .= "<label class='col-xs-12 col-md-6 col-sm-12 userplus-checkbox'><span";
                        $res = '';
                        if ((is_array($value) && in_array($v, $value)) || $v == $value) {
                            $res = 'checked';
                        }
                        $disabled = (!empty($readonly)) ? 'disabled' : $readonly;
                        $field .= '></span><input class="'.$res.'" '.$is_required.' '.$res.' type="checkbox" value="' . $v . '" name="' . $array['meta_key'] . '[]" ';
                        $field .= " $disabled /> ".ucwords($v)."</label>";
                    }
                    $field .= "</div>";
                }
                break;
        ...
        $className = $array['meta_key'];
        $field .= '<span for="'. $elementId .' " id="error' . $elementId . '" class="wpuser_error wpuser_error_'.$className.'" style="display: none"></span>';
        if(isset($array['description']) && !empty($array['description'])){
            $field .= '<p>'.$array['description'].'</p>';
        }
