    ...
    class wpuserAdminAjax
    {
        ...
        public function wpuser_update_page_setting()
        {
            ...
            if ($cu->has_cap('manage_options')) {
                ...
                $post = array(
                    ...
                    'post_name' => sanitize_text_field( $data['wp_user_page_title'] ),
                    'post_status' => 'publish',
                    'post_title' => sanitize_text_field( $data['wp_user_page_title'] ),
                ...
                $post = array(
                    ...
                    'post_author' => 1,
                    'post_date' => date('Y-m-d H:i:s'),
                    'post_name' => sanitize_text_field( $data['wp_user_member_page_title'] ),
                    'post_status' => 'publish',
                    'post_title' => sanitize_text_field( $data['wp_user_member_page_title'] ),
        ...
        public function wpuser_bulk_process()
        {
            ...
            if ($cu->has_cap('manage_options')) {
                ...
                $userlist_arr = isset( $_POST['userlist'] ) ? (array) $_POST['userlist'] : array();
                $userlist_arr = $this->recursive_sanitize_text_field( $userlist_arr );

                $responce = array(
                    'status' => 1,
                    'message' => 'Setting has been updated',
                    'userlist' => $userlist_arr,
                    'bulk_action' => sanitize_text_field( $_POST['bulk_action'] )
                );

                if ($_POST['bulk_action'] == 'Export') {
                    $fields = (isset($_REQUEST['include_fields']) && !empty($_REQUEST['include_fields'])) ? explode(',', sanitize_text_field( $_REQUEST['include_fields'] )) : array('ID', 'user_login', 'user_nicename', 'display_name', 'user_email', 'user_activation_key', 'user_registered');
                    $include_users = (isset($_POST['userlist']) && !empty($_POST['userlist'])) ? (array) $_POST['userlist'] : array();
                    $include_users = $this->recursive_sanitize_text_field( $include_users );
                    $args = array(
                        'role' => '',
                        'role__in' => array(),
                        'role__not_in' => array(),
                    ...
                    $include_users_arr = (isset($_POST['userlist']) && !empty($_POST['userlist'])) ? (array) $_POST['userlist'] : array();
                    $include_users_arr = $this->recursive_sanitize_text_field( $include_users_arr );
                    foreach ($include_users_arr as $userlist) {
