...
function wpuser_metabox_setting()
{
    ...
    foreach ($metabox['fields'] as $field) {
        ...
        echo '<tr>';
        echo '<th style="width:20%"><label for="' . esc_attr($field['id']) . '">' . esc_attr( $field['name'] ). '</label></th>';
        echo '<td>';
        switch ($field['type']) {
            case 'select':
                echo '<select name="' . esc_attr($field['id']) . '" id="' . esc_attr($field['id']) . '">';
                foreach ($field['options'] as $option => $label) {
                    echo '<option' . selected($meta, $option, false) . ' value="' . esc_attr($option) . '">' . esc_attr( $label ). '</option>';
                }
                echo '</select>';
                break;
        }
        echo '<td>' . esc_attr( $field['desc'] ). '</td><td>';
        echo '</tr>';
    }
    echo '</table>';
}
